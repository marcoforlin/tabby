...
use crate::commands::tantivy_provider::IndexReaderProvider;
...
impl CodeSearchImpl {
    ...
    async fn search_in_language(
        ...
        let out = merge_code_responses_by_rank(reader, &params, docs_from_embedding, docs_from_bm25)
            .await;
        Ok(
            out
        )
...
async fn merge_code_responses_by_rank(
    ...
    let bm25_rank = compute_rank_score(bm25_resp.clone());

    for (rank, bm25, doc) in bm25_rank.into_iter() {
...
impl CodeSearch for CodeSearchService {
    async fn search_in_language(
        ...
        let reader_guard = self.provider.reader().await;
        if let Some(reader) = reader_guard.as_ref() {
            return self.imp.search_in_language(reader, query, params).await
        } else {
